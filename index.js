...
(function () {
  ...
  G.init = function () {
    ...
    function finishInit() {
      ...
      G.utils.ajax({ method: 'GET', url: G.API_URL + '/sites' }, function (err, data) {
        if (err) {
        ...
        G.utils.ajax({ method: 'GET', url: G.API_URL + '/news' }, function (errNews, dataNews) {
          if (!errNews) {
          ...
          if (path === '/' || path === '/index.html') {
            ...
            // Add a guard to prevent a logged-in user from accessing the registration page.
            if (G.state.loggedInSiteData) {
              // This is a state conflict that crashes IE6. Redirect to the main portal view.
              G.state.view = 'portal';
            } else {
